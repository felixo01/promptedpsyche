# Articles Publication Notes

## First Articles

The first article pair published in pre-launch mode is:
...
- site-wide `noindex` remains active
- Vercel `X-Robots-Tag` remains active
- publication does not mean search indexing is enabled

## Editorial Scope

The articles link the public article layer with Concepts Wave 02. They use the concepts as a vocabulary for working with AI beyond prompt writing:
...
An optional `TL;DR` / `W skrócie` disclosure is used for long-form articles.
...
- The public labels are `TL;DR` in EN and `W skrócie` in PL.
- The internal field may remain `inBrief`.
...
Prompted Psyche does not assign fake DOI identifiers.

A DOI can be added only after registration through a real registration route such as Crossref, DataCite, Zenodo or another legitimate repository or agency.

Until then, every public article should use:

- a visible author byline
- a canonical URL
- a suggested citation
- a rights notice under the article

The DOI field should not be displayed when no DOI exists. Do not show placeholder DOI text.
