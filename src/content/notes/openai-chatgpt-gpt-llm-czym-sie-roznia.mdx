...
Nie znaczy to, że użytkownik zawsze ma odtworzyć całą przyczynę techniczną. Powinien jednak unikać wniosku `OpenAI się pomyliło, więc każdy GPT jest zły` albo przeciwnego `ChatGPT ma źródła, więc odpowiedź jest prawdziwa`. Hasło [Odpowiedź modelu](/pl/concepts/model-output/) pomaga traktować wynik jako materiał wymagający oceny odpowiedniej do stawki.
