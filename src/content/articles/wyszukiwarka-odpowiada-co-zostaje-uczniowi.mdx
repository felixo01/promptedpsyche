---
title: "Jak zmienia się proces uczenia się, gdy wyszukiwarka podaje gotową odpowiedź?"
description: "Jak generatywna wyszukiwarka przejmuje część wyboru źródeł, porównywania dowodów i syntezy oraz jak zmienia przez to pracę ucznia."
...
inBrief:
  - "Generatywne wyszukiwanie zmienia nie tylko formę odpowiedzi, lecz także to, kto wybiera źródła, porównuje dowody i tworzy syntezę."
  - "Audyt Common Sense Media z 2026 roku pokazuje możliwości Google Search w testowanych konfiguracjach kont w USA, a nie długoterminowe efekty uczenia się u uczniów korzystających z produktu na co dzień."
  - "Gotowa synteza może ułatwić wykonanie zadania, a jednocześnie ograniczyć okazję do ćwiczenia oceny materiałów i uzasadniania wniosku."
...
<p class="lede article-deck">Gotowa synteza oszczędza czas, ale może też przejąć wybór źródeł, porównanie dowodów i część pracy, dzięki której uczeń buduje własny osąd.</p>

<aside class="editorial-aside" data-qa="publication-method">
  <strong>Metoda i zakres.</strong> To oparty na badaniach esej koncepcyjny i synteza narracyjna. Literatura została dobrana selektywnie, nie systematycznie; nie było prerejestrowanej strategii wyszukiwania ani ilościowego oszacowania efektu. Przypadek Google jest datowanym studium produktu. Tekst rozdziela wyniki badań, ramę interpretacyjną autora i propozycje projektowe.
</aside>
...
Punktem wyjścia jest audyt opublikowany w lipcu 2026 roku przez Common Sense Media Youth AI Safety Institute. Organizacja sprawdziła dwie odrębne funkcje Google Search: AI Overviews, czyli podsumowania pojawiające się automatycznie dla części zapytań, oraz AI Mode, osobny tryb konwersacyjny wybierany przez użytkownika. Raport dotyczy amerykańskiej wersji produktu testowanej od 19 maja do 1 lipca 2026 roku w rejonie Zatoki San Francisco.[^2]

Badacze korzystali z dwóch typów kont testowych zarządzanych przez Family Link, z aktywnym SafeSearch. Pierwszy typ reprezentował jedenastolatka objętego aktywnym nadzorem rodzicielskim, drugi zaś piętnastoletniego członka grupy rodzinnej bez takiego nadzoru. Audytorzy przeprowadzili ponad 2600 interakcji w siedmiu planach testowych, obejmujących między innymi zadania szkolne, pytania historyczne, aktualność informacji, uprzedzenia i materiały syntetyczne. Liczby w tabeli raportu sumują się do 2624. W badaniu nie uczestniczyli jednak uczniowie korzystający z wyszukiwarki na co dzień; audytorzy pracowali według przygotowanych scenariuszy.
...
## 3. Wykonanie zadania a uczenie się
...
Badanie Hamsy Bastani i współautorów pokazuje, dlaczego warto oddzielać wynik uzyskany z pomocą od późniejszej samodzielnej pracy. Eksperyment objął niemal tysiąc uczniów jednej szkoły średniej w Turcji i składał się z czterech krótkich sesji. Standardowy dostęp do GPT poprawił wyniki podczas ćwiczeń matematycznych, ale grupa ta wypadła gorzej w testach bez AI przeprowadzanych w ramach tych samych sesji. Wersja z zabezpieczeniami pedagogicznymi ograniczyła ten efekt. Badanie nie dotyczyło Google Search i mierzyło krótkoterminową zdolność samodzielnego wykonania zadań. Pokazuje jednak, że lepszy wynik z narzędziem nie musi oznaczać lepszego wyniku bez niego.[^4]

Równie ważny jest kontrprzykład. Badanie Grega Kestina i współautorów objęło 194 studentów fizyki i dwie lekcje. Ustrukturyzowany tutor wykorzystujący AI pomógł osiągnąć lepszy wynik w testach bezpośrednio po zajęciach niż zajęcia oparte na aktywnym uczeniu. Narzędzie prowadziło studentów przez materiał, zamiast tylko podawać końcową odpowiedź.[^5] To wynik z konkretnego kursu i określonej grupy, bez mocnego pomiaru trwałości efektu. Pokazuje jednak, że problemem nie jest sama obecność AI, lecz rodzaj pomocy i jej dopasowanie do celu zadania.

## 4. Praca między pytaniem a odpowiedzią
...
Nie ma powodu idealizować tego procesu. Użytkownicy klasycznego internetu także otwierali pierwszy wynik, zatrzymywali się na krótkim fragmencie albo kopiowali gotowy materiał. Jeszcze przed upowszechnieniem generatywnej AI badania uczniów pokazywały trudności z oceną autorstwa, interesów stojących za stroną i jakości dowodów.[^6] Problem nie zaczął się od AI. Gotowa synteza może jednak zmniejszać liczbę okazji do zauważenia i przećwiczenia tych trudności.

ACRL opisuje kompetencje informacyjne między innymi jako strategiczne szukanie oraz ocenę autorytetu źródła zależnie od kontekstu.[^7] Nie wystarczy znaleźć zdanie pasujące do pytania. Trzeba wiedzieć, dlaczego dane źródło jest tu istotne, jaki rodzaj wiedzy wnosi i gdzie są jego granice. Badania nad czytaniem bocznym (lateral reading), czyli wyjściem poza ocenianą stronę i sprawdzeniem jej w innych miejscach, pokazują, że osoby zawodowo weryfikujące informacje robią to częściej niż pozostali badani.[^8]
...
Płynność przetwarzania (processing fluency) oznacza łatwość, z jaką odbieramy informację. W klasycznym eksperymencie Rolfa Rebera i Norberta Schwarza łatwość odczytania zdania wpływała na ocenę jego prawdziwości.[^12] To psychologiczne tło dla pytania o płynnie sformułowaną syntezę, nie bezpośredni dowód dotyczący [odpowiedzi AI](/pl/concepts/model-output/), dzieci czy Google Search. Można natomiast powiedzieć pewnie, że profesjonalny wygląd nie informuje, jak mocne są przesłanki odpowiedzi.
...
<aside class="editorial-aside" data-qa="related-reading">
  <strong>Powiązane teksty w Prompted Psyche.</strong> Czytaj dalej: [Kiedy AI wspiera uczenie się, a kiedy tylko wyręcza?](/pl/articles/nie-pytaj-czy-ai-nas-oglupia/), [Nie chodzi tylko o prompt](/pl/articles/nie-chodzi-tylko-o-prompt/) i [Zaufanie do nauki w erze gotowych odpowiedzi](/pl/articles/zaufanie-w-epoce-gotowych-odpowiedzi/).
</aside>
...
[^10]: Audyt cytowań i kategorie jakości: pozycja 1, s. 21. Treści użytkowników nie są w tym tekście utożsamiane z fałszem.
